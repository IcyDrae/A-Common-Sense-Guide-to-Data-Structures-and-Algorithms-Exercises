...
(the previous number in the sequence). Write a function that accepts a number
for N and returns the correct number from the series. That is, if
...
*/
        public int sequence(int number)
        {
            ...
            return number + sequence(number - 1);
        }
    }
}
